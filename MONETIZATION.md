...
3. **Free credit grant on signup — ✅ path decided 2026-06-06: recurring free tier (revised from "$2 one-time"; claw-back killed the one-time silent path). Verify `rollover:false` refresh before launch.**
   ~6–8 substantial sessions: enough to *feel* the hosted model before paying — the funnel hinge. Gate behind OAuth signup + a per-IP velocity limit. The abuse ceiling is low at ~$0.75 (75 cr), so don't over-build the guard (see Tier 3).
...
Branch `fix/polar-credits-meter-boot-guard` (work committed). Re-verified 2026-06-09: server `tsc` exit 0; `bun test` 132/132; the provision + verify scripts `tsc`-clean + smoke-tested.
...
- **Left before launch:** (1) provision in sandbox + run the server to confirm a fresh user lands 75 (needs a *sandbox* credits meter — the `.env` meter is prod); (2) the `rollover:false` per-cycle refresh — **the shared-meter netting half is now verified (sandbox, 2026-06-09 via `scripts/verify-tiers.ts`): free + Pro + PAYG grants stack to 1075 credited and a 50-credit debit nets the pool to 1025.** The per-cycle *refresh* is the only open billing item and is **time-gated**: a `--keep` run (2026-06-09) parked a live sandbox customer `darkcode-tier-verify-1781053755673` (meter `867558ae…`, `interval=month`) for the re-check. The script only does `month`/`year` intervals (no sub-month shortcut), so close it out by re-reading after the renewal (~2026-07-09): `bun run verify:tiers --check darkcode-tier-verify-1781053755673` (with `POLAR_SERVER=sandbox` + that `POLAR_CREDITS_METER_ID`) — PASS = the allowance resets rather than stacks. Safe to launch ahead of it: the worst case (credits stack instead of reset) over-grants free/Pro credits — a detectable margin leak, not a customer-billing bug — so it can equally be confirmed by watching `creditedUnits` on the first real renewal; (3) known minor race — a brand-new user's first hosted turn can hit the gate before the async grant lands (sub created in ~1s, credit shortly after) — harden later by awaiting in the gate or a dedicated post-login claim endpoint; (4) **✅ DONE — per-IP velocity abuse guard** (`lib/free-grant-guard.ts`): soft cap of 10 new grants/IP/24h on the Redis-or-memory store, counts only landed grants (peeking never increments, so shared NAT/CGNAT returning users aren't penalised), fails open; over-cap attempts audit `billing.free_tier_blocked`.

**✅ Item 4 — Pro $20/mo subscription + premium-model tiering (scaffolded 2026-06-08; inert until provisioned).** Ships safely off until `POLAR_PRO_PRODUCT_ID` is set: with it unset, `/pro` 503s and premium models stay available to anyone on credits (today's behavior). Server `tsc` exit 0; `bun test` green (+10 in `routes/pro-gate.test.ts`); both Polar provision scripts `tsc`-clean.
...
- **Left before launch:** (1) provision Pro in *sandbox* + run a Pro checkout to confirm the included credits land on the meter and the gate flips (premium models go from "credits" to "needs Pro"); (2) runtime-verify the per-cycle credit refresh (shares the `rollover:false` open item with Item 3 — same meter); (3) tune the exact included-credit allotment (`POLAR_PRO_CREDITS`) — **the shared-meter question is now settled: `bun run verify:tiers` confirmed (sandbox, 2026-06-09) that free 75 + Pro 900 + PAYG 100 stack additively (credited 1075) and a 50-credit `darkcode_usage` debit nets the combined pool to 1025**, so free + Pro + PAYG can share the prod meter with no read-path change (a revoke claws back only that grant's units — persistent PAYG order credits are unaffected); (4) the Pro check adds one `subscriptions.list` call per premium hosted turn — cache it (Redis/idempotency) if it shows up in latency.
...
**Still open (decide when building, not blocking):** the **free-tier monthly amount + interval** (suggest ~50–100 credits/month — well under Pro's allotment — at `month` interval); the **`rollover:false` per-cycle *refresh* is still NOT empirically verified** — before launch confirm a grant resets to the allotment each cycle (time-gated: `verify:tiers --keep`, then `--check` after a renewal). *Shared-meter netting is now verified* (sandbox 2026-06-09: free+Pro+PAYG stack to 1075, a debit nets to 1025), so sharing the prod meter is safe — the "separate meter" sidestep is no longer needed; exact included-credit allotment for Pro (start ~800–1000 ≈ $8–10); the annual-discount %; whether PAYG top-ups ever need a small markup.
